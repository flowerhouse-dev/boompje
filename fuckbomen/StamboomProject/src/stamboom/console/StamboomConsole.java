...
public class StamboomConsole {
    ...
    void OpslaanOfLaden() throws IOException{
        File testOpslag = new File("testOpslag");
        String g = readString("Opslaan of Laden (o/l)");
            if (g.toLowerCase().charAt(0) == 'o') {
                controller.serialize(testOpslag);
                return;
            }
            if (g.toLowerCase().charAt(0) == 'l') {
                controller.deserialize(testOpslag);
            }
   
    }
    
    void toonStamboom() throws IOException{
        Persoon p = selecteerPersoon();
