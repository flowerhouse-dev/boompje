...
public class Persoon implements java.io.Serializable {
 ...
 //       throw new UnsupportedOperationException();
    ...
    public String getNaam() {
        ...
        {
            naam += " " + this.tussenvoegsel + " " + this.achternaam;
        }
        else
        {
    ...
    public String getVoornamen() {
        String voornamenstring = "";
        for (String s : voornamen) {
            s = s.replaceAll(" ","");
            s = StringUtilities.withFirstCapital(s);
            voornamenstring += StringUtilities.withFirstCapital(s) + " ";
        }
        voornamenstring = voornamenstring.substring(0,voornamenstring.length()-1);
        return voornamenstring;
    }
    ...
    public Gezin heeftOngehuwdGezinMet(Persoon andereOuder) {
        for (Gezin gezin : alsOuderBetrokkenIn) {
            if(gezin.isOngehuwd())
            {
                if (gezin.getOuder1() == andereOuder || gezin.getOuder2() == andereOuder || andereOuder == null)
                {
                    System.out.println("gezin gevonden");
                    return gezin;
                }
            }
    ...
    public int afmetingStamboom() {
        ...
        {
        int aantal = this.ouderlijkGezin.getOuder1().afmetingStamboom() + 1;  
        return aantal; 
        }
        if(this.ouderlijkGezin.getOuder2()!= null)
        {
        int aantal = this.ouderlijkGezin.getOuder1().afmetingStamboom() + this.ouderlijkGezin.getOuder2().afmetingStamboom() + 1;
        return aantal;
        }
        return 1;
    }
    ...
    void voegJouwStamboomToe(ArrayList<PersoonMetGeneratie> lijst, int g) {
        ...
        {
            Gezin G = this.getOuderlijkGezin();
            G.getOuder1().voegJouwStamboomToe(lijst, g + 1);
            if(G.getOuder2() != null)
            {
             G.getOuder2().voegJouwStamboomToe(lijst, g + 1);
            }           
        }
    }
     ...
     * ______I.T.M.A. Goldsmid (VROUW) 2-12-1876<br>
    ...
    public String stamboomAlsString() {
        ...
        {
            String spaties = new String();
            for(int i = 0; i < p.getGeneratie() ; i++)
            {
                spaties += "  ";
            }
            builder.insert(builder.length(),spaties + p.getPersoonsgegevens() + System.getProperty("line.separator"));
        }
